...
class process_data():
    def __init__(self,yaml_path,origin_input,input,output):
        ...
        # self.denormalize = self.get_denormalize()
    ...
    def read_data(self,x):
        ...
        #data_x[data_x<-10000]=0 #将陆地部分置为0
        #data_x = self.transforms(data_x) #归一化
        # print(data_x.shape)
        #return data_x.numpy()
...
#     # x = np.random.rand(1,52,2041,4320).astype(np.float32)
#     x = torch.rand(1,52,2041,4320)
#     yaml_path = '/home/data/wrz/GpSwin/configs/data.yaml'
#     x=process_data(yaml_path,'input_1to22','ouput_1to22').read_data(x)
